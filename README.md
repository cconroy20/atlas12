...
- Molecular equilibrium network extended and repaired.  The species list (`molecules.dat`) gains CrO, CrS, TiN, TiS, and TiH, refit from Barklem & Collet (2016) Table 7 equilibrium constants in the same physical-form basis as the existing "BC16 fit" rows via the new `tools/fit_molecule_keq.py`, which is kept in-repo so the fit provenance survives with the data (its `--validate` mode regenerates all 41 existing neutral-diatomic BC16 rows exactly).  Li, Be, and B — previously in the network only as neutrals bound in LiH/BeH/BH, with no ionized sink in element conservation — gain ionization stages I–III.  The Y I and Zr I ionization potentials in `ionpots.dat` are corrected against BC16 Table 4 / NIST (both were ~0.2 eV high, a ~2× overestimate of the neutral fractions at 3500 K).  Solar models are unaffected; effects are confined to cool atmospheres, where CrO reaches parity with CrH in the upper photosphere and TiH depletes deep-layer TiO by ~2% at 2500 K, moving the emergent flux by ≲0.5%
